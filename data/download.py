...
def download_and_process_datasets(directory, datasets):
  ...
  tf.logging.info("Preparing Speech dataset: {}".format(
      ",".join(datasets)))
  for dataset in datasets:
    ...
    convert_audio_and_split_transcript(
        dataset_dir + "/Speech", dataset, dataset + "-wav",
        dataset_dir + "/Speech", dataset + ".csv")


def define_data_download_flags():
  """Define flags for data downloading."""
  absl_flags.DEFINE_string(
      "data_dir", "/tmp/speech_data",
...
def main(_):
  ...
  if FLAGS.train_only:
    ...
    download_and_process_datasets(FLAGS.data_dir, ["test-other"])
  else:
